-- Consulta para obter os registros da tabela "demonstracoes_contabeis" que atendem a certos critérios

SELECT reg_ans, vl_saldo_final  -- Seleciona as colunas "reg_ans" e "vl_saldo_final" para serem exibidas no resultado
FROM demonstracoes_contabeis  -- Especifica a tabela alvo da consulta como "demonstracoes_contabeis"
WHERE descricao IN ('EVENTOS/ SINISTROS CONHECIDOS OU AVISADOS  DE ASSISTÊNCIA A SAÚDE MEDICO HOSPITALAR ', 'EVENTOS/ SINISTROS CONHECIDOS OU AVISADOS  DE ASSISTÃŠNCIA A SAÃšDE MEDICO HOSPITALAR ')  -- Filtra as linhas onde a coluna "descricao" está presente em determinados valores
    AND data BETWEEN '2022-01-01' AND '2022-12-31'  -- Filtra as linhas onde a coluna "data" está dentro do intervalo especificado (entre '2022-01-01' e '2022-12-31')
